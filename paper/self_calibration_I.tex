...
\begin{abstract}
We study the self-calibration---determination of complex system
response from science data alone---for precise photometric catalogs
from wide-field imaging surveys. We create an artificial sky of stars
...
and a complex, position-dependent instrument response. We then use a
self-calibration technique to recover the instrument response by
simultaneously fitting for all the star fluxes and the parameters of a
position-dependent flat-field, optimizing a justified likelihood
function.  For realism, we deliberately fit with a wrong noise model
and a flat-field functional basis that does not include the model that
generated the synthetic data.  We demonstrate that with a favorable
survey strategy, a complex (but smooth) instrument response can be
precisely self-calibrated. We also find that returning the same
sources to very different focal plane positions is the key property of
any survey strategy designed for accurate retrospective
calibration. The results of this work suggest the following advice for
those considering the design of large-scale imaging surveys: Do not
use a regular tiling of the sky; instead return the same sources to
very different focal plane positions.
\end{abstract}
...
In this work we do not produce pixelated images.  Instead we create catalog-level simulations with realistic measurement uncertainties, including unmodeled stellar variability.  Complex effects are included in the simulations that are (deliberately) not correctly modeled at the analysis stage, in order to simulate the effects of unknown systematic errors within the catalog.
...
All of the code used in this paper is publicly available at
\url{http://github.com/davidwhogg/SelfCalibration/}~.
