...
\begin{abstract}
In this paper, we investigate under which survey conditions the self-calibration of an instrument response can produce precise photometric catalogs for wide-field imaging surveys, drawing only on the science data themselves. We consider four basic survey strategies and create end-to-end simulations of an imaging survey, which produce simulated photometric catalogs from mock observations of a realistic, synthetic sky. These catalog-based simulations include realistic measurement uncertainties and a complex, position-dependent instrument response. We then use a self-calibration technique to recover the instrument response by fitting an model that best explains the survey catalog, based on the sources that fall on different focal plane positions in separate observations. We demonstrate that with a favorable survey strategy, a complex (but smooth) instrument response can be precisely calibrated using this self-calibration technique. We also find that returning the same sources to very different focal plane positions is the key property of any survey strategy designed for accurate retrospective calibration. The results of this work suggest the following advice for those considering the design of large-scale imaging surveys: Do not use a regular tiling of the sky; instead return the same sources to very different focal plane positions. 
\end{abstract}
...
Rory Holmes was funded by the Deutschen Zentrums f\"ur Luft- und Raumfahrt (DLR) through the grant 50QE1202.
