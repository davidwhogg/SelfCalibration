...
def plot_survey(params, survey_filename):
  ...
  plt.bar(hist_mag[:-1], np.histogram(mag, bins = hist_mag)[0] / (area) , width = 0.5, color = 'k', alpha =0.3)
  plt.xlabel(r'Source Magnitude $m$ (mag$_\textrm{\small{AB}}$)')
  plt.ylabel(r'Density of Sources ((0.5 mag$_\textrm{\small{AB}}$)$^{-1}$ deg$^{-2}$)')
